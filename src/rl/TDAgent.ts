...
import { randi, sampleWeighted, zeros } from "./utils";
...
class TDAgent {
    ...
    beta=0.01;// learning rate for policy, if smooth updates are on
    lambda=0;// eligibility trace decay. 0 = no eligibility traces used
    replacing_traces=true;
    q_init_val=0;// optional optimistic initial values
    planN=0;// number of planning steps per learning iteration (0 = no planning)
    Q:Float64Array;
    P;
    e;
    env_model_s:Float64Array;
    env_model_r;
    env;
    ...
    reset() {
        ...
        if (this.q_init_val !== 0) { this.Q.fill(this.q_init_val); }
        this.P = zeros(this.ns * this.na);
        this.e = zeros(this.ns * this.na);

        // model/planning vars
        this.env_model_s = zeros(this.ns * this.na);
        this.env_model_s.fill(-1);// init to -1 so we can test if we saw the state before
